...
<html lang="es">
    <head>
        ...
        <title>Actualizar Pintura</title>
        <link rel="stylesheet" href="css/bootstrap.min.css">
        <link rel="stylesheet" href="css/misestilos.css"/>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        <link href="css/styles.css" rel="stylesheet">
    </head>
    <body id="body-actualizar-pintura">
        <div id="actualizar-pintura-container">
            <div id="actualizar-pintura-background"></div>
            <div id="actualizar-pintura-box">
                <h1>Actualizar Pintura</h1>

                <form method="post" action="ServletPinturaa" onsubmit="return validarFormulario()">
                    <input type="hidden" name="instruccion" value="actualizarPintura">
                    <input type="hidden" name="idPintura" value="<%= ((Pintura) request.getAttribute("pintura")).getIdPintura()%>">

                    <div class="form-group">
                        <label for="nombre">Nombre de la Pintura</label>
                        <input type="text" class="form-control" name="nombre" id="nombre" value="<%= ((Pintura) request.getAttribute("pintura")).getNombre()%>" pattern="[a-zA-Z\s]+" title="El nombre solo debe contener letras y espacios." required>
                    </div>

                    <div class="form-group">
                        <label for="fechaCreacion">Fecha de Creación</label>
                        <input type="date" class="form-control" name="fechaCreacion" id="fechaCreacion" value="<%= new java.text.SimpleDateFormat("yyyy-MM-dd").format(((Pintura) request.getAttribute("pintura")).getFechaCreacion())%>" max="<%= new java.text.SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date())%>" required>
                    </div>

                    <div class="form-group">
                        <label for="valorEstimado">Valor Estimado</label>
                        <input type="number" class="form-control" min="0" name="valorEstimado" id="valorEstimado" max="10000000" step="0.01" value="<%= ((Pintura) request.getAttribute("pintura")).getValorEstimado()%>" step="0.01" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="pintor" class="custom-select" required>
                            ...
                            %>
                            <option value="<%= tempPintor.getIdpintor()%>" <%= tempPintor.getIdpintor() == ((Pintura) request.getAttribute("pintura")).getPintor() ? "selected" : ""%>><%= tempPintor.getNombre()%></option>
                            <%
                                }
                            %>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="tecnica" class="custom-select" required>
                            ...
                            %>
                            <option value="<%= tempTecnica.getIdTecnica()%>" <%= tempTecnica.getIdTecnica() == ((Pintura) request.getAttribute("pintura")).getTecnica() ? "selected" : ""%>><%= tempTecnica.getTecNombre()%></option>
                            <%
                                }
                            %>
                        </select>
